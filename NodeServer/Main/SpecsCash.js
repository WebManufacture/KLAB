...
SpecsCash.Init = function(){
	...
	SpecsCash.Server = new NodeTunnel('http://web-manufacture.net:12222');
	...
	if (localStorage['spec'] && localStorage['spec'].length > 10){
		...
		SpecsCash.Server.all('http://web-manufacture.net:12222', function(rt, er){
...
SpecsCash.SyncFromServer = function(){
	if (localStorage['lst']){
		...
		var syncRequestUrl = Request.GetUrl('http://web-manufacture.net:12222/spec/', {_lastmodified: localStorage['lst']});
		...
		//var newChSp = [];
		 ...
		 while (chSp.length > 0){
			...
			if (spec['_id'][0] != 'n'){
				...
				var newSpecUrl = Request.GetUrl('http://web-manufacture.net:12222/spec/', {_id: idDB});
				...
				spec['_lastmodified'] = (new Date).getTime();
				var newSpecData = JSON.stringify(spec);
				SpecsCash.Server.add('http://web-manufacture.net:12222/spec/', newSpecData, 'text/plain', SpecsCash._addSpecFunction(idToUpdate));	
			};
		};		
	}
	else{
...
SpecsCash.KillSpec = function(spec){
	...
	if (spec['_id'][0] != 'n'){
		...
		//TestSpecsDB.Server.add('http://web-manufacture.net:12222/spec/', newSpecData, 'text/plain', TestSpecsDB._addSpecFunction(idToUpdate));	
	};
};
...
SpecsCash.AddSpec = function(){
	for (var i = 0; i < SpecsCash.UN.length; i++){
		...
		SpecsCash.Server.add('http://web-manufacture.net:12222/sync/', spec, 'text/plain',  function(){
