    ...
    Win.LoadWindow = function(url){
	url = url.toLowerCase();
	M.OnModuleRegistered.add(Win.WindowModuleLoaded, url)
	M.LoadModule(url);
    };
	
Win.WindowModuleLoaded = function(url, module){
    var win = Win.CreateNewWindow();
    win.setTitle(url);
    var visual = module.get(".module-visual");
    if (visual != null){
	while (visual.childNodes.length > 0){
	    win.content.add(visual.childNodes[0]);
	}
    }
};
    ...
    Win.CreateNewWindow = function(windowType){
	  ...
	  windowType = "ui_standart_window"; 
        ...
        win.cls("window");
        win.rcs("prototype");
        var title = win.get(".window_title");
        if (title != null){
          win.title = title.html();
        }
	win.setTitle = Win.SetTitle;
        win.id = "win" + Win.Items.length;
        ...
        win.header = win.get(".window_header");
        if (win.header != null){
	    if (win.header.attr("draggable") == "true"){
		Drag.MakeDraggable(win.header); 
	    }	    
           win.header.click = Win.MakeTop;
        }
	...
	Win.SetTitle = function(title){
	    var titleElem = this.get(".window_title");
	    if (titleElem != null){
