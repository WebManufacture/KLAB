...
Request = {
    Params: { },  CreateUrl: function(file, param0, param1, param2, param3, param4, param5, param6) {
...
String.prototype.endsWith = String.prototype.end = function(str) 
{
...
if (window.W == undefined || window.W == null || !window.W.IsOlmObject) {
    ...
    /*if (window.Node && window.XMLSerializer && Element.prototype.outerHTML == undefined) {
	...
	WrapPrototype: function(elem){
	    elem.wrap = this.Wrap;
	    elem.ToString = this.ToString;
	    elem.toObj = this.ToObj;
	    elem.tag = this.tag;
	    elem.cls = this.cls;
	    elem.rcs = this.rcs;
	    elem.get = this.get;
	    ...
	    elem.All = elem.all = elem.findAll = this.findAllInternal;
	...
	Wrap : function(html, filter){
	    if (Check(html) && typeof(html) == "string"){
		var elem = W.div("", html);
		if (filter != undefined){
	...
	ToString : function(onlyHeader){
	    var result = this.sname;
	    if (this.attr("id") != null){
		result += "#" + this.attr("id");
	    }
	    result += "." + this.attr("class").replace(/ /g, ".");
	    if (onlyHeader) return result;
	},
	...
	prev : function(path){
	    if (path == null) return this.previousSibling;
	    var next = this.previousSibling;
	    while (next != null){
		if (next.nodeType == 1 && next.is(path)){
		    return next;
		}
	...
	next : function(path){
	    if (path == null) return this.nextSibling;
	...
	last : function(path){
	    try{
		var elems = this.childs(path);
		if (elems.length > 0) return elems[elems.length - 1];
	    }
	    catch(e){
		L.LogError(e, "InvalidSelector: " + path); 
	    }
	...
	All : function(path,wrap){
	    return W.findAll(path, wrap);
	},
	...
	aget: function(attr, value, path){
	    ...
	    path += "]";
	    if (this == W){
		return W.Body.querySelector(path);
	    }
	    else{
		return this.querySelector(path);
	    }
	...
	MapEvent : function(selector, eventType, handler){
	    var result = W.findAll(selector);
	    for (var i = 0; i < result.length; i++){
		result[i].eadd(eventType, handler);
	    }
	    return result;
	},
...
if (window.E == undefined || window.E == null || !window.E.IsOlmObject) {
	...
	Event = {
		...
		for (var i=0; i < this.handlers.length; i++){
		    ...
		    if (handler.condition != undefined && handler.condition != null){
			if (handler.condition == argument1){
			    this.result = this.handle(handler, argument1, argument2);
			    if (E.Debug)
				L.LogInfo("HANDLER: " + this.eventName + " : " + argument1 + " : " + handler.condition, "System.JS.js", "events");
			}
		    }
		    else{
			this.result = this.handle(handler, argument1, argument2);
		...
		try
		{		    
		    if (typeof(handler) != "function" && handler[this.eventName] != undefined){
