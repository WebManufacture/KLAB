if (!using("Ax")) {
	...
	ax.GetUrl = function(url, urlparams) {
	    if (url.contains("/") && !url.start("http://")) {
		url = "http://" + url;
	    }
	...
	ax.AddUrlParam = function(url, paramName, value) {
	    ...
	    if (value)
	    {
		paramName = paramName + "=" + escape(value);
	    }
	    url += paramName;
	    return url;
	};
	...
	ax.EndRequest = function(req) {
	    if (ax.onAjaxFinish){
		ax.onAjaxFinish(this.type, this);
	    }
	    var result = true;
	...
	Server.Init = server.init = function(handler, root) {
	    ...
	    this.CrossDomain = this.cd = this.SystemRoot != window.location.href
	...
	Server.GetSystemUrl = function(url, urlparams) {
	    if (this.CrossDomain && !url.start("http://")){
	...
	Server.GetHandlerUrl = function(urlparams) {
	    var url = this.GetSystemUrl(this.handler, urlparams);
	    if (Request.SystemPath){
		url = ax.AddUrlParam(url, "path", Request.SystemPath);
	    }
	    return url;
	};
	...
	Server.Request = function(action, selector, data, callback, params, urlparams) {
	...
	Server.Command = function(action, selector, data, callback, params, urlparams) {
	...
	Server.Init("system.default.ashx");
	...
	SysAjax.LoadModule = function(fileName, context, cache, callback) {
	    if (cache) {
		if (this.CrossDomain) {
		    var rq = this.Request('get', fileName, null, callback, {context: context}, {rnd : Math.random()})
			rq.start();
		    return rq;
		}
		else{
		    return Server.Get(fileName, callback, {context: context}, {rnd : Math.random()});
		}
	    }
	    else{	 
		if (this.CrossDomain) {
		    return this.Get(fileName, callback, {context: context});
		}
		else{
		    return Server.Get(fileName, callback, {context: context});
		}
	    }
	};
    }
...
WS.DOMload(function(){
	...
	AjaxJasp.JaspRequest = function(action, file, selector, data, callback, context) {
	    return this.Command(action, selector, data, callback, {context: context}, {file: file});
	};
	...
	AjaxJasp.Del = AjaxJasp.det = function(file, selector, callback, context) {
	...
	AjaxJasp.Add = AjaxJasp.add = function(file, selector, data, callback, context) {
	    return AjaxJasp.JaspRequest('append', file, selector, data, callback, context);
	};
