if (!UsingDOM("Modules")) {
    ...
    M.GetModuleStatus = function (url) {
        ...
        if (mod.is(".inprogress")) return "inprogress";
        if (mod.is(".processed")) return "processed";
        return "unknown";
    };
    ...
    M.GetModuleContainsUrl = function (url) {
        url = url.toLowerCase();
        return M.get(".module[url*='" + url + "']", ".module");
    };
    ...
    M.CreateModule = function (url, state) {
        var module = M.div(".module");
        module.add("." + state);
        url = url.toLowerCase();
        module.url = url;
        module.set("@url", module.url);
    ...
    M.ScriptExists = function (ourl) {
        ...
        var docScript = DOM.get(selector.replace(/1/g, ourl));
        if (docScript != null) {
        ...
        var docScript = DOM.get(selector.replace(/1/g, url));
        if (docScript != null) {
    ...
    M.WaitContext.Process = function (element, context, module, from) {
        ...
        if (url != null) {
            if (element.is(".inprogress")) return false;
	    url = url.toLowerCase();
	    element.url = url;
	...
	var waits = M.all("wait[url='" + url + "']");
	for (var i = waits.length - 1; i >= 0; i--){
	    var wait = waits[i];
	    wait.rcs("inprogress");
	    wait.cls("processed");
	    module = waits[i].get("^.module");
	    if (module){
	...
	if (module.wait) return false;
        ...
        if (url != null) {
            if (script.is(".inprogress")) return false;
            script.del();
	...
	if (module.wait) return false;
        if (script.is(".deffered")) return true;
	...
	var result = M.Load(element.url, module.url, null);
        ...
        if (result && result.is(".inprogress")) {
            element.cls("inprogress");
	...
	if (module.wait) {
        ...
        var inprogress = module.all(".inprogress");
	...
	if (!module.isScript) {
	    C.Process(module, "ui-processing", module);
            var elems = module.all(">*:not(.processed):not(.ui-processed):not(.module-element):not(script):not(link):not(style):not(title):not(meta)");
            for (var i = elems.length - 1; i >= 0; i--) {
                var elem = WS.Body.add(elems[i]);                
            }
        }
	...
	var includings = DOM.all("include.inprogress[url='" + url + "']");
        ...
        var parent = module.get("^.module.inprogress");
        if (parent) {
