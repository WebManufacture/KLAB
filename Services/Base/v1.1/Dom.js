...
if ((window.WS == undefined || window.WS == null) && (window.DOM == undefined || window.DOM == null || window.DOM.JaspVersion < 3.15)) {
    ...
    DOM.Func = DOM.func = function (path, callback) {
	...
	if (path && path.length > 0 && path.start) {
		...
		for (var i = 0; i < this.childNodes.length; i++){
		    ...
		    if (node && node.nodeType == 1 && node.is(path)) 
			return node;
		}		
		return null;
	    }
	...
	return this.get(selector + "[" + path + "='" + value + "']");
    };
...
return wrapper.firstChild;*/
    ...
    DOM.ToString = DOM.toString = function(withBody){
